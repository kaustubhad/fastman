...
if (length(col2)==1) {
  if (col2=="matlab") { col2=c("#D95319","#E4A100","#7E2F8E","#5EA500","#0095D4","#A2142F","#0C53AA"); }
  ...
  else if (col2=="greys") { col2=c("gray35","gray60"); }
  ...
  else { warning("inappropriate col2 color vector/tag provided. resorting to default"); col2=c("#D95319","#E4A100","#7E2F8E","#5EA500","#0095D4","#A2142F","#0C53AA"); } # use matlab as default
}
...
if (bybp) { # show mb position instead of scaled position. scale positions to make suitable for rounding. applies if single chromosome
  ...
  cmat=matrix(NA,nrow=numc,ncol=3); # a matrix cmat is created with number of rows as number of unique chr and 3 columns
  for (i in 1:numc) { ch=unc[i]; f=m$CHR==ch; w=m$BP[f]; cmat[i,1]=min(w); cmat[i,2]=max(w); cmat[i,3]=cmat[i,2]-cmat[i,1]; m$BP[f]=w-cmat[i,1]; }
  cmat=as.data.frame(cmat); colnames(cmat)=c("min","max","width"); # each row of cmat corresponds to each chr, 3 columns of cmat are calculated as min BP, max BP and width
