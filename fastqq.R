fastqq <- function(p, speedup=TRUE, lambda=TRUE, fix_zero=TRUE, cex=0.6, cex.axis=0.9, xlab, ylab, ...) {
...
# fix p=zero cases, if necessary by setting them minimum/2
f=(p==0);
if (any(f)) {
if (fix_zero) { mx=min(p[!f]); p[f]=mx/2; }
else { p=p[!f]; }
}
...
# part 3: sort and create expected distribution
